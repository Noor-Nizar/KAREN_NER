    ...
    for tokenized_input, label_ids in zip(tokenized_inputs, tokenized_labels):
        ...
        for token, word_idx, label_id in zip(tokens, word_ids, label_ids):
            if word_idx is None:
                ...
                sentence[-1] += token[2:] if token.startswith('##') else token
                sentence_labels[-1] = label_id  # Maintain the original label
...
def decode_and_align_labels_main(tokenized_inputs, tokenized_labels, tokenizer):
    sentences = []
    labels = []
    ...
    for tokenized_input, label_ids in zip(tokenized_inputs, tokenized_labels):
        ...
        for token, word_idx, label_id in zip(tokens, word_ids, label_ids):
            if word_idx is None:
                ...
                # Start of a new word
                token = token.lstrip("▁")  # Remove the underscore prefix for the start of a new word
                sentence.append(token)
                sentence_labels.append(label_id)
            else:
                # Continuation of the same word (subword token)
                token = token.lstrip("▁")  # Remove the underscore, as it's a continuation
                sentence[-1] += token
                # Label stays the same (we do not reassign it)
